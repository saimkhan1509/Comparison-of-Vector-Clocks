...
int findtime(int processid, std::vector<int>receivedvector){

    for(int i=0;i<receivedvector.size();i=i+2){
            if(receivedvector[i]==processid)
                return receivedvector[i+1];
    }
    return 1;
}


// Creates update vector (as an STL string) which is to be sent to other nodes/threads

std::string kalyanivectortostring(std::vector<int> vectortime,std::vector<int> LS,std::vector<int> LU, int p, int& l){
    std::stringstream st;
    for(int i=0;i<vectortime.size();i++){
        if(LS[p]<LU[i]){
            st<<i;
...
std::vector<int> kalyanistringtovector (std::string str,int n){
    std::stringstream s(str);
    std::vector<int> vec;
    int num;
    char c;
...
void* node_func(void* arg)
{
    ...
    int* noofentriessent = (static_cast<argument_class_node*>(arg))->ptr2;
    ...
    noofentriessent[j]=0;                                       // noofentriessent[j] = number of update entry pairs sent by node j till now
    ...
    std::vector<int> LS(n,0);                               // Last Sent vector
    std::vector<int> LU(n,0);                               // Last Update vector

    std::vector<int> receivedvector(n,0);
    std::string sendvectorstring;                           // Update vector to be sent
    std::string myvectortimestring;                         // Vector Clock in STL string form
    char buffer[2*n*11];                                    // Buffer to receive update vector
    ...
    while(l<m){
        ...
        for(int t=0;t<numerator;t++){       // INTERNAL EVENTS
            ...
            myvectortimestring=vectortostring(myvectortime);

            printf("Process %d executes internal event e%d%d at %llu:%llu:%llu:%llu, vc: [%s]\n", j+1, j+1,myvectortime[j],
                   reqtime/3600000,reqtime%3600000/60000,reqtime%60000/1000,reqtime%1000,myvectortimestring.c_str());
        ...
        for(int q=0;q<denominator && l<m;q++){          // SEND EVENTS
            myvectortime[j]+=1;
            LS[j]=myvectortime[j];
            LU[j]=myvectortime[j];
            ...
            sendvectorstring=kalyanivectortostring(myvectortime,LS,LU,adjacencylist[j][p]-1,noofentriessent[j]);
            myvectortimestring=vectortostring(myvectortime);
            ...
            if(status!=sendvectorstring.size()){
                ...
                LS[adjacencylist[j][p]-1]=myvectortime[j];
        ...
        for(int i=1;i<adjacencylist[j].size();i++){                  // RECEIVE EVENTS
            if(FD_ISSET(sockarr2[(adjacencylist[j][i]-1)*n+j],&readysockets)){
                ...
                receivedvector=kalyanistringtovector(buffer,n);
                myvectortime[j]+=1;
                LS[j]=myvectortime[j];
                LU[j]=myvectortime[j];
                for(int k=0;k<receivedvector.size();k=k+2){         // Compare vector clock with received update vector and update
                    if (myvectortime[receivedvector[k]]<receivedvector[k+1]){
                            myvectortime[receivedvector[k]]=receivedvector[k+1];
                       LU[receivedvector[k]]=myvectortime[j];
                    }
                }
                reqtime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
                reqtime = (reqtime + 19800000) %86400000;                   //A day consists of 86400000 milliseconds. 19800000 added to adjust international time delay.
                myvectortimestring=vectortostring(myvectortime);
                printf("Process %d receives message m%d%d from process %d at %llu:%llu:%llu:%llu, vc: [%s]\n", j+1, adjacencylist[j][i],findtime(adjacencylist[j][i]-1,receivedvector),adjacencylist[j][i],
                       reqtime/3600000,reqtime%3600000/60000,reqtime%60000/1000,reqtime%1000,myvectortimestring.c_str());
...
int main()
{
  ...
  /* --------READING PARAMETERS---------*/
    ...
    FILE* fptr;
    fptr = fopen ("outputfile2.txt","w");

    int datasent[n+1];
    int noofentriessent[n+1];
...
/* --------CREATING ONE CLIENT AND ONE SERVER THREAD---------*/
    ...
    pthread_create(&creator_threads[0], NULL, creator_func1, (void*)&arguments1);                   // Creating threads
    pthread_create(&creator_threads[1], NULL, creator_func2, (void*)&arguments2);                   // Creating threads
...
/* --------INITIALISING ARRAY TO PASS PARAMETERS---------*/
    ...
    argument_class_node arguments[n];                 // Array to pass the parameters to the SAM1 & SAM2 threads
    for (int j=0;j<n;j++)
    {
        arguments[j].j=j;
        arguments[j].n=n;
        ...
        arguments[j].ptr2=noofentriessent;
...
/* --------PRINTING SPACE USAGE OUTPUT TO CONSOLE AND LOG FILE---------*/
    ...
    for(int i=0;i<n;i++){
        ...
        noofentriessent[n]+=noofentriessent[i];
        printf("\nProcess %d\nSpace utilised by to store\ni) Vector Clocks: %lu bytes\nii) LS[] and LU[]: %lu bytes\nSize of the messages sent: %d bytes\nNumber of entries sent: %d"
           ,i+1,n*sizeof(int),n*sizeof(int)*2,datasent[i],noofentriessent[i]);
        fprintf(fptr,"\nProcess %d\nSpace utilised by to store\ni) Vector Clocks: %lu bytes\nii) LS[] and LU[]: %lu bytes\nSize of the messages sent: %d bytes\nNumber of entries sent: %d"
           ,i+1,n*sizeof(int),n*sizeof(int)*2,datasent[i],noofentriessent[i]);
    }

    printf("\n\nTotal space utilised to store\ni) Vector Clocks: %lu bytes\nii) LS[] and LU[]: %lu bytes\nTotal size of the messages sent: %d bytes\nTotal number of entry pairs sent: %d"
           ,n*n*sizeof(int),n*n*sizeof(int)*2,datasent[n],noofentriessent[n]);
    fprintf(fptr,"\nTotal space utilised to store\ni) Vector Clocks: %lu bytes\nii) LS[] and LU[]: %lu bytes\nTotal size of the messages sent: %d bytes\nTotal number of entry pairs sent: %d"
           ,n*n*sizeof(int),n*n*sizeof(int)*2,datasent[n],noofentriessent[n]);
